...
/* Read rdata for type UNECE and ISO. */
int32_t read_unece_iso_rdata(struct domain_table *domains, uint16_t rdlength,
	struct buffer *packet, struct rr **rr);

/* Print rdata for type UNECE and ISO. */
int print_unece_iso_rdata(struct buffer *output, const struct rr *rr);
