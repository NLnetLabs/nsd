...
void cookie_verify(query_type *q, struct nsd* nsd, uint32_t *now_p) {
	...
	if(q->remote_addr.ss_family == AF_INET6) {
		memcpy(q->edns.cookie + 16, &((struct sockaddr_in6 *)&q->remote_addr)->sin6_addr, 16);
		verify_size = 32;
	} else {
		memcpy(q->edns.cookie + 16, &((struct sockaddr_in *)&q->remote_addr)->sin_addr, 4);
		verify_size = 20;
	}
#else
	memcpy( q->edns.cookie + 16, &q->remote_addr.sin_addr, 4);
	verify_size = 20;
#endif
...
void cookie_create(query_type *q, struct nsd* nsd, uint32_t *now_p)
{
	...
	if (q->remote_addr.ss_family == AF_INET6) {
		memcpy( q->edns.cookie + 16
		      , &((struct sockaddr_in6 *)&q->remote_addr)->sin6_addr, 16);
		siphash(q->edns.cookie, 32, nsd->cookie_secrets[0].cookie_secret, hash, 8);
	} else {
		memcpy( q->edns.cookie + 16
		      , &((struct sockaddr_in *)&q->remote_addr)->sin_addr, 4);
